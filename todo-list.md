...
4.【待完成】完善记忆机制的生命周期:
- 温层摘要替身持久化：3-15天内存档只留存摘要、元数据、关键词等替身
- 冷层压缩与向量化归档：15天以上删除原文，只保留摘要向量和元数据
- 夜间自动归档/压缩：每天凌晨对超过期限的文件进行归档，生成摘要替身并把摘要向量化存入向量库
- 重要性评分驱动的分层驻留：实现LLM自动打分（1-5分），高分信息长期驻留在全局记忆或温层，低分信息执行冷层压缩
- 时间指向型 Query Router：根据用户问题的时间指向性智能选择检索层（热/温/冷/全局）
- 知识图谱（GraphRAG）：将用户偏好、人物关系、项目状态等结构化信息提取为节点和边

5.【待完成】构建Skills系统:
- 实现skills文件夹更新后的自动监听与重建索引机制
- 将skills知识库真正接入技能选择/触发流程
- 实现宿主机外部目录监听与热更新
- 完善技能自动匹配和触发机制

6.【待完成】优化系统性能和稳定性:
- 实现温层摘要替身 + 冷层压缩入口 + Query Router
- 完善Query Router功能
- 实现skills目录变更监听和skill命中接线
- 完善第二阶段RAG的重排序/压缩/忠实度检查
...
优先级建议：
1. 首先完成温层摘要替身 + 冷层压缩入口 + Query Router
2. 然后实现skills目录变更监听和skill命中接线
3. 最后完善第二阶段RAG的rerank/compression/faithfulness
