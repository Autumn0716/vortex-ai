...
export function MemoryTimelinePanel({
  ...
  const timeline = useMemo(() => {
    ...
    return entries;
  }, [agentId, auditEntries, documents, filter]);
    ...
    <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-4">
      ...
      {!loading && timeline.length ? (
        <div className="mt-4 space-y-3">
          {timeline.slice(0, 18).map((entry) => (
            <div key={`${entry.source}:${entry.id}`} className="relative pl-5">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3">
                ...
                <div className="mt-2 line-clamp-2 text-xs leading-5 text-white/58">{entry.detail}</div>
              </div>
            </div>
          ))}
        </div>
