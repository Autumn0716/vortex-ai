...
test('ensureAgentWorkspaceSchema adds reviewer branch id to task graphs', () => {
  const runs: string[] = [];
  ...
  const database = {
    ...
    exec(sql: string) {
      ...
      if (sql === 'PRAGMA table_info(topic_task_graphs)') {
        return [
          {
            columns: ['name'],
            values: [
  ...
  assert.ok(runs.some((sql) => sql.includes('ALTER TABLE topic_task_graphs ADD COLUMN reviewer_branch_topic_id TEXT')));
});
